...
async def handle_rule_query(query: str) -> dict[str, str]:
    q = (query or "").strip()
    if not q:
        return {
            "response": (
    ...
    if any(k in lowered for k in ("player count", "players online", "concurrent", "how many players", "players in")):
        ...
        return {"response": text, "tool": "steam_stats", "mode": "rules"}
    ...
    if "achievement" in lowered:
        text = await _call("steam_stats", {"operation": "achievements", "steamid": "", "app_id": app_id})
        return {"response": text, "tool": "steam_stats", "mode": "rules"}
    ...
    if "review" in lowered:
        ...
        return {"response": text, "tool": "steam_store", "mode": "rules"}
    ...
    if "workshop" in lowered:
        ...
        text = await _call("steam_workshop", {"operation": "query", "app_id": app_id, "query": search, "count": 10})
        return {"response": text, "tool": "steam_workshop", "mode": "rules"}

    if any(k in lowered for k in ("search", "find game", "look up game", "store")):
        search = _extract_query(q) or q.replace("search", "").replace("for", "").strip() or "indie"
